...
enum class EnumExample1 {
    A, B, C // Enum constants are separated with commas.
}
fun printEnum() = println(EnumExample1.A) // => A
...
fun printProperty() = println(EnumExample2.A.value) // => 1

// Every enum has properties to obtain its name and ordinal(position) in the enum class declaration:
fun printName() = println(EnumExample2.A.name) // => A
fun printPosition() = println(EnumExample2.A.ordinal) // => 0
